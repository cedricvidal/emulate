...
Repositories are clonable and pushable over Git smart HTTP. `git` must be on PATH, because the
transport runs `git upload-pack` and `git receive-pack`.

```bash
git clone http://localhost:4001/octocat/hello-world.git
```

Set `github.git_dir` in the seed, or `EMULATE_GIT_DIR`, to choose where the bare mirrors live.
Without either, a temporary directory is created and removed on exit.

## Using the gh CLI

`gh` addresses exactly one host over plain HTTP, `github.localhost`, and reads its token from
...
The MCP server accepts plain HTTP only for a loopback host, so point it at `localhost` directly. Go
never proxies `localhost`, so it bypasses `HTTP_PROXY` and both clients can be configured at once:
...
REST is served at the root and under `/api/v3`, GraphQL at `/graphql` and `/api/graphql`, and raw
file content at `/raw/:owner/:repo/:ref/:path`.

## Importing a real repository

`scripts/import-github` snapshots a real repository into seed state, including full commit history,
every issue and pull request with comments, and the label set.
...
import contains it with issue #6, "Slack Support", still open and pull request #10 not yet merged.

`--ref` rewrites the mirror so a clone checks out that commit and carries only its history. A pinned
snapshot implies a point in time, so issue and pull request state is rewound to the ref's commit date
too: anything created later is left out, and anything closed or merged later comes back as open. Use
`--as-of <iso>` for a different moment, or omit `--ref` to import current state.

With a token the import is a single GraphQL query. Without one it falls back to REST, which allows
only 60 requests per hour for anonymous callers.

## Resetting between runs

`POST /_emulate/reset` clears the store, discards the repository mirrors so a pushed branch cannot
leak into a later run, and re-applies the seed.
